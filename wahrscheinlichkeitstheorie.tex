...
\hlc{Student's $t$-Verteilung - $T \sim t(n)$}: Seien $X \sim N(0, 1)$ und $Z \sim \chi^2(n)$ unabhängige Zufallsvariablen. Dann folgt die Zufallsvariable $T = \frac{X}{\sqrt{Z/n}}$. $E(T) = 0$ für $n \ge 2$ und $Var(T) = \frac{n}{n-2}$ für $n \ge 3$. \emph{Mögliche Anwendungen}: Für kleine $n$ deutlich \glqq spitzer\grqq\, als $N(\mu, \sigma^)$. Kann für dinge verwendet werden, die weniger um $\mu$ schwanken. Läuft aber gegen eine Normalverteilung für große $n$\\
